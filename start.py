...
SYMCC_BIN = "/AFLsymcc/symcc/build/symcc"
...
def start_symcc(task):
    ...
    cmd = "SYMCC_TARGET={}_symcc AFL_CUSTOM_MUTATOR_LIBRARY=/AFLsymcc/AFLplusplus/custom_mutators/symcc/symcc-mutator.so afl-fuzz -i work_dir/{}/input -o work_dir/{}/output {}_afl >> log/{}.log".format(binary, task.split("/")[-1], task.split("/")[-1], binary, task.split("/")[-1])
    ...
    while True:
        ...
        if time.time() > timeout:
            ...
            kill(p.pid)
            break
        # timeout = time.time() + 60*15 # 15 min
        # poll = p.poll()
        # if poll is not None:
            #print(task)
            #print("timeout!")
...
if __name__ == '__main__':
    ...
    async_result = p.map(start_symcc, tasks)
    p.close()
    p.join()
    make_zip()
